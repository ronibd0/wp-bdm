...
const ResponsiveColorComponent = (props) => {
			...
			<div className="ast-color-btn-reset-wrap">
				<button
					...
					disabled={
						...
						JSON.stringify(props.control.params.default)
					}
					onClick={(e) => {
						e.preventDefault();
	...
	const renderSettings = (key) => {
		return (
			<AstraColorPickerControl
				color={
					undefined !== props_value[key] && props_value[key]
						? props_value[key]
						: ""
				}
	...
	const { label, description, responsive } = props.control.params;
	...
	return (
		<div className="ast-control-wrap">
			...
			<div className="customize-control-content">{inputHtml}</div>
		</div>
	);
};
