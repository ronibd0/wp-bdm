...
describe( 'Customizing html element in the above header section', () => {
	it( 'text color for HTML element should apply correctly', async () => {
		...
		//text color
		await page.waitForSelector( '.ast-header-html-1 .ast-builder-html-element' );
		await expect( {
	...
	it( 'link color for HTML element should apply correctly', async () => {
		...
		await expect( {
			...
			property: 'color',
		} ).cssValueToBe( `${ htmlBlock[ 'header-html-1link-color' ].desktop }`,
		);
		await setBrowserViewport( 'medium' );
		await page.waitForSelector( '.ast-header-html-1 a' );
		await expect( {
			selector: '.ast-header-html-1 a',
			property: 'color',
		} ).cssValueToBe( `${ htmlBlock[ 'header-html-1link-color' ].tablet }`,
		);
		await setBrowserViewport( 'small' );
		await page.waitForSelector( '.ast-header-html-1 a' );
		await expect( {
			selector: '.ast-header-html-1 a',
			property: 'color',
		} ).cssValueToBe( `${ htmlBlock[ 'header-html-1link-color' ].mobile }`,
		);
	} );
