...
function astra_container_layout_css() {
	...
	if ( 'page-builder' === $container_layout ) {
		...
		if ( true === $customizer_default_update ) {
			...
			if ( 'disabled' !== $display_title && true === apply_filters( 'astra_stretched_layout_with_spacing', true ) && false === $enable_trans_header && false === astra_check_any_page_builder_is_active( astra_get_post_id() ) ) {
				/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				$page_container_css .= '
