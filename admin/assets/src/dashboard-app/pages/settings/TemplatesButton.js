...
const TemplatesButton = () => {
    ...
    const updateEnableTemplatesStatus = () => {
		...
		const action = 'uag_enable_templates_button',
			nonce = astra_admin.enable_templates_button_nonce;
        ...
        <section className='block border-b border-solid border-slate-200 px-12 py-8 justify-between'>
            <div className='mr-16 w-full flex items-center'>
                <h3 className="p-0 flex-1 justify-right inline-flex text-lg leading-8 font-medium text-gray-900">
                    { __( 'Enable Templates Button', 'astra' ) }
                </h3>
                <Switch
            ...
            <p className="mt-2 w-9/12 text-sm text-slate-500">
                { __( 'Spectra comes with a stunning library of page templates and block patterns. This library is accessible with the Templates button while editing the page or post. Manage the visibility of that button with this option.', 'astra' ) }
            </p>
        </section>
    );
};
