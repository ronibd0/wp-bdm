...
import { setCustomize, uploadImage, setLogo } from '../../../../utils/customize';
...
describe( 'Site logo settings in the customizer', () => {
	it( 'site logo width should apply correctly', async () => {
		...
		await expect( {
			...
			property: 'max-width',
		} ).cssValueToBe( `${ sitelogoWidth[ 'ast-header-responsive-logo-width' ].desktop }px` );

		await setBrowserViewport( 'medium' );
		await expect( {
			selector: 'header .custom-logo-link img',
			property: 'max-width',
		} ).cssValueToBe( `${ sitelogoWidth[ 'ast-header-responsive-logo-width' ].tablet }px` );
