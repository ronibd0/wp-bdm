...
const ComingSoon = () => {
	...
	const updateenableComingSoonMode = () => {
		...
		formData.append( 'security', astra_admin.enable_coming_soon_mode_nonce );
	...
	const updateSelectedPage = ( page ) => {
		...
		formData.append( 'security', astra_admin.coming_soon_page_nonce );
	...
	const fetchPageHandler = ( keyword = '' ) => {
		...
		formData.append( 'security', astra_admin.fetch_pages_nonce );
		...
		<section className='block border-b border-solid border-slate-200 px-12 py-8 justify-between'>
			<div className='mr-16 w-full flex items-center'>
				<h3 className="p-0 flex-1 justify-right inline-flex text-lg leading-8 font-medium text-gray-900">
					{ __( 'Enable Coming Soon Mode', 'astra' ) }
				</h3>
				<Switch
			...
			<p className="mt-2 w-9/12 text-sm text-slate-500">
				{ __( 'Is your website still in the making? Is it not yet ready for other people to see?', 'astra' ) }
			</p>
			<p className="mt-2 w-9/12 text-sm text-slate-500">
				{ __( 'Enable the coming soon mode. Only the page you select below will be displayed to people who are not logged in.', 'astra' ) }
			</p>
			<Select
				isMulti={ false }
